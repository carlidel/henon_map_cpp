...
__device__ void tangent_matrix(const double &x, const double &px,
                               const double &y, const double &py,
                               const double &sx, const double &cx,
                               const double &sy, const double &cy,
                               const double &mu, double *out, size_t idx) {
...
__device__ void random_4d_kick(double &x, double &px, double &y, double &py,
                               const double &kick_module, curandState &state) {
  double rand_kicks[4];
  ...
  double mod =
      sqrt(rand_kicks[0] * rand_kicks[0] + rand_kicks[1] * rand_kicks[1] +
           rand_kicks[2] * rand_kicks[2] + rand_kicks[3] * rand_kicks[3]);
...
__device__ void henon_step_gpu(double &x, double &px, double &y, double &py,
                           unsigned int &steps, const double *sinx,
                           const double *cosx, const double *siny,
                           const double *cosy, const double &barrier_pow_2,
                           const double &mu, const double &kick_module,
                           curandState &state, const bool reverse,
                           const unsigned int &allowed_steps) {
  if (check_nan(x, px, y, py)) {
    return;
  }

  int modulo_steps = steps;
  if (modulo_steps >= allowed_steps) {
    modulo_steps = modulo_steps % allowed_steps;
  }
  if (modulo_steps <= 0) {
    if (allowed_steps == 1) {
      modulo_steps = 1;
      ...
      do {
        modulo_steps += allowed_steps;
      } while (modulo_steps <= 0);
    }
  }
  ...
  if (!reverse) {
    ...
    rotation(x, px, sinx[modulo_steps], cosx[modulo_steps], reverse);
    rotation(y, py, siny[modulo_steps], cosy[modulo_steps], reverse);
  } else {
    rotation(x, px, sinx[modulo_steps - 1], cosx[modulo_steps - 1], reverse);
    rotation(y, py, siny[modulo_steps - 1], cosy[modulo_steps - 1], reverse);
...
__global__ void gpu_evaluate_radius(const double *x, const double *px,
                                    const double *y, const double *py,
                                    double *r, int size) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < size) {
    r[idx] = sqrt(x[idx] * x[idx] + px[idx] * px[idx] + y[idx] * y[idx] +
                  py[idx] * py[idx]);
  }
}

__global__ void gpu_evaluate_action(const double *x, const double *px,
                                    const double *y, const double *py,
                                    double *r, int size) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < size) {
    r[idx] = (x[idx] * x[idx] + px[idx] * px[idx] + y[idx] * y[idx] +
              py[idx] * py[idx]);
  }
}

__global__ void gpu_evaluate_action_x(const double *x, const double *px,
                                      const double *y, const double *py,
                                      double *r, int size) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < size) {
...
__global__ void gpu_evaluate_action_y(const double *x, const double *px,
                                      const double *y, const double *py,
                                      double *r, int size) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < size) {
...
__global__ void gpu_evaluate_angle(const double *x, const double *px,
...
__global__ void gpu_compute_displacements(double *x1, double *px1, double *y1,
                                          double *py1, double *out,
                                          const size_t size,
                                          const double low_value = 1,
                                          const bool log_me = false) {
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= size) {
    return;
  }

  if (check_nan(x1[i], px1[i], y1[i], py1[i]) ||
      check_nan(x1[i + size], px1[i + size], y1[i + size], py1[i + size])) {
    out[i] = NAN;
    return;
  }
  ...
  if (log_me) {
    out[i] = log(displacement(x1[i], px1[i], y1[i], py1[i], x2, px2, y2, py2) /
                 low_value);
  } else {
...
__global__ void gpu_compute_displacements_and_realign(double *x1, double *px1,
                                                      double *y1, double *py1,
                                                      double *out,
                                                      const size_t size,
                                                      const double low_module) {
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= size) {
    return;
  }
  ...
  double tmp_displacement =
      displacement(x1[i], px1[i], y1[i], py1[i], x1[i + size], px1[i + size],
                   y1[i + size], py1[i + size]);
  out[i] += log(tmp_displacement / low_module);
  realign(x1[i + size], px1[i + size], y1[i + size], py1[i + size], x1[i],
          px1[i], y1[i], py1[i], tmp_displacement, low_module);
}

__global__ void gpu_add_to_ratio(double *new_displacement,
                                 double *old_displacement, double *ratio,
...
__global__ void gpu_sum_two_arrays(double *in1, const double *in2,
                                   const size_t size) {
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= size) {
    return;
...
__global__ void gpu_henon_track(
    double *g_x, double *g_px, double *g_y, double *g_py, int *g_steps,
    const size_t n_samples, const unsigned int max_steps,
    const double barrier_pow_2, const double mu, const double *omega_x_sin,
    const double *omega_x_cos, const double *omega_y_sin,
    const double *omega_y_cos, const double kick_module, curandState *state,
    const bool reverse, const unsigned int allowed_steps) {
  size_t j = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  if (j < n_samples) {
    ...
    for (unsigned int k = 0; k < max_steps; ++k) {
      henon_step_gpu(x, px, y, py, steps, omega_x_sin, omega_x_cos, omega_y_sin,
                 omega_y_cos, barrier_pow_2, mu, kick_module, state[j], reverse,
                 allowed_steps);
    }
...
__global__ void get_matrices_and_multiply(double *matrices, const double *x,
                                          const double *px, const double *y,
                                          const double *py, const double *sx,
                                          const double *cx, const double *sy,
                                          const double *cy, const double mu,
                                          const unsigned int global_steps,
                                          const int size) {
  ...
  tangent_matrix(x[idx], px[idx], y[idx], py[idx], sx[global_steps],
                 cx[global_steps], sy[global_steps], cy[global_steps], mu,
                 tmp_matrix, 0);
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      ...
      for (int k = 0; k < 4; k++) {
        result_matrix[i * 4 + j] +=
            tmp_matrix[i * 4 + k] * matrices[idx * 16 + k * 4 + j];
      }
    }
  }
...
__global__ void get_matrices_and_set(double *matrices, const double *x,
                                     const double *px, const double *y,
                                     const double *py, const double *sx,
                                     const double *cx, const double *sy,
                                     const double *cy, const double mu,
...
__global__ void matrix_vector_multiply(const double *matrices, double *vectors,
                                       const size_t size) {
  size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void gpu_particle_renormalize(double *x, double *px, double *y,
                                         double *py, const size_t *idx,
  ...
  double init_module =
      displacement(x[i], px[i], y[i], py[i], x[idx_base[i]], px[idx_base[i]],
                   y[idx_base[i]], py[idx_base[i]]);

  realign(x[i], px[i], y[i], py[i], x[idx_base[i]], px[idx_base[i]],
          y[idx_base[i]], py[idx_base[i]], init_module, module);
}

void particles_4d_gpu::renormalize(const double &module_target) {
  gpu_particle_renormalize<<<_optimal_nblocks(), 512>>>(
      d_x, d_px, d_y, d_py, d_idx, d_idx_base, d_ghost, x.size(),
      module_target);
}

__global__ void
gpu_particle_displacements(const double *x, const double *px, const double *y,
                           const double *py, double *out, const size_t *idx,
                           const size_t *idx_base, const uint8_t *ghost,
                           const size_t size, const size_t n_ghosts) {
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if (check_nan(x[i], px[i], y[i], py[i]) ||
      check_nan(x[idx_base[i]], px[idx_base[i]], y[idx_base[i]],
                py[idx_base[i]])) {
    out[idx_base[i] + pad * idx[i]] = NAN;
    return;
  }

  out[idx_base[i] + pad * idx[i]] =
      displacement(x[i], px[i], y[i], py[i], x[idx_base[i]], px[idx_base[i]],
                   y[idx_base[i]], py[idx_base[i]]);
}

__global__ void
gpu_particle_directions(const double *x, const double *px, const double *y,
                        const double *py, double *out_x, double *out_px,
                        double *out_y, double *out_py, const size_t *idx,
                        const size_t *idx_base, const uint8_t *ghost,
                        const size_t size, const size_t n_ghosts) {
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  realign(out_x[idx_base[i] + pad * idx[i]], out_px[idx_base[i] + pad * idx[i]],
...
__global__ void kernel_lyap_birk(double *values, double *values_b,
                                 const double *vectors, const double *weights,
                                 size_t size, size_t n_weights, size_t b_idx) {
  size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= size)
    return;
...
__global__ void kernel_megno(double *d_layer1, double *d_layer2,
                             double *d_layer3, const double *d_matrix_a,
                             const double *d_matrix_b, size_t N, size_t idx) {
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= N)
    return;
  ...
  for (size_t j = 0; j < 4; j++) {
    for (size_t k = 0; k < 4; k++) {
      trace_a +=
          d_matrix_a[i * 16 + j * 4 + k] * d_matrix_a[i * 16 + j * 4 + k];
    }
  }
  ...
  for (size_t j = 0; j < 4; j++) {
    for (size_t k = 0; k < 4; k++) {
      trace_b +=
          d_matrix_b[i * 16 + j * 4 + k] * d_matrix_b[i * 16 + j * 4 + k];
    }
  }
...
__global__ void kernel_megno_birkhoff(double *d_layer1, double **d_layer2,
                                      double **d_weights,
                                      const size_t *d_turn_samples,
                                      const double *d_matrix_a,
                                      const double *d_matrix_b, size_t N,
                                      size_t idx, double n_turn_samples) {
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= N)
    return;
...
__global__ void kernel_tune_birkhoff_init(double *d_x, double *d_px,
                                          double *d_y, double *d_py,
...
__global__ void kernel_tune_birkhoff(double *d_x, double *d_px, double *d_y,
                                     double *d_py, double *store_x,
                                     double *store_y, double **d_tune1_x,
                                     double **d_tune2_x, double **d_tune1_y,
                                     double **d_tune2_y, double **d_weights,
                                     const size_t *d_turn_samples, size_t N,
                                     size_t idx, double n_turn_samples) {
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= N)
    return;
...
void particles_4d_gpu::_general_cudaMalloc() {
  ...
  cudaMalloc(&d_steps, steps.size() * sizeof(int));
...
void particles_4d_gpu::_general_host_to_device_copy() {
  ...
  cudaMemcpy(d_px, px.data(), px.size() * sizeof(double),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_y, y.data(), y.size() * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(d_py, py.data(), py.size() * sizeof(double),
             cudaMemcpyHostToDevice);

  cudaMemcpy(d_steps, steps.data(), steps.size() * sizeof(int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_valid, valid.data(), valid.size() * sizeof(uint8_t),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_ghost, ghost.data(), ghost.size() * sizeof(uint8_t),
             cudaMemcpyHostToDevice);

  cudaMemcpy(d_idx, idx.data(), idx.size() * sizeof(size_t),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_idx_base, idx_base.data(), idx_base.size() * sizeof(size_t),
             cudaMemcpyHostToDevice);
}

void particles_4d_gpu::_general_device_to_host_copy() {
  cudaMemcpy(x.data(), d_x, x.size() * sizeof(double), cudaMemcpyDeviceToHost);
  cudaMemcpy(px.data(), d_px, px.size() * sizeof(double),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(y.data(), d_y, y.size() * sizeof(double), cudaMemcpyDeviceToHost);
  cudaMemcpy(py.data(), d_py, py.size() * sizeof(double),
             cudaMemcpyDeviceToHost);

  cudaMemcpy(steps.data(), d_steps, steps.size() * sizeof(int),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(valid.data(), d_valid, valid.size() * sizeof(uint8_t),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(ghost.data(), d_ghost, ghost.size() * sizeof(uint8_t),
             cudaMemcpyDeviceToHost);

  cudaMemcpy(idx.data(), d_idx, idx.size() * sizeof(size_t),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(idx_base.data(), d_idx_base, idx_base.size() * sizeof(size_t),
             cudaMemcpyDeviceToHost);
}
...
particles_4d_gpu::particles_4d_gpu(const std::vector<double> &x_,
                                   const std::vector<double> &px_,
                                   const std::vector<double> &y_,
                                   const std::vector<double> &py_)
...
void particles_4d_gpu::add_ghost(const double &displacement_module,
...
particles_4d_gpu::get_displacement_module() const {
  std::vector<std::vector<double>> val_displacement(
      n_ghosts_per_particle,
  ...
  gpu_particle_displacements<<<_optimal_nblocks(), 512>>>(
      d_x, d_px, d_y, d_py, d_out, d_idx, d_idx_base, d_ghost, x.size(),
      n_ghosts_per_particle);

  for (size_t i = 0; i < n_ghosts_per_particle; i++) {
    cudaMemcpy(val_displacement[i].data(), d_out + i * n_particles,
               n_particles * sizeof(double), cudaMemcpyDeviceToHost);
  }

  std::vector<std::vector<double>> val_displacement_transposed(
      n_particles,
      std::vector<double>(n_ghosts_per_particle,
                          std::numeric_limits<double>::quiet_NaN()));
...
particles_4d_gpu::get_displacement_direction() const {
  ...
  gpu_particle_directions<<<_optimal_nblocks(), 512>>>(
      d_x, d_px, d_y, d_py, d_out_x, d_out_px, d_out_y, d_out_py, d_idx,
      d_idx_base, d_ghost, x.size(), n_ghosts_per_particle);

  for (size_t i = 0; i < n_ghosts_per_particle; i++) {
    cudaMemcpy(dir_displacement[0][i].data(), d_out_x + i * n_particles,
               n_particles * sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(dir_displacement[1][i].data(), d_out_px + i * n_particles,
               n_particles * sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(dir_displacement[2][i].data(), d_out_y + i * n_particles,
               n_particles * sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(dir_displacement[3][i].data(), d_out_py + i * n_particles,
               n_particles * sizeof(double), cudaMemcpyDeviceToHost);
  }
...
const std::vector<double> particles_4d_gpu::get_x() const {
  ...
  // copy data to host
  cudaMemcpy(x_copy.data(), d_x, x.size() * sizeof(double),
             cudaMemcpyDeviceToHost);
  return x_copy;
}
...
const std::vector<double> particles_4d_gpu::get_px() const {
  ...
  // copy data to host
  cudaMemcpy(px_copy.data(), d_px, px.size() * sizeof(double),
             cudaMemcpyDeviceToHost);
  return px_copy;
}
...
const std::vector<double> particles_4d_gpu::get_y() const {
  ...
  // copy data to host
  cudaMemcpy(y_copy.data(), d_y, y.size() * sizeof(double),
             cudaMemcpyDeviceToHost);
  return y_copy;
}
...
const std::vector<double> particles_4d_gpu::get_py() const {
  ...
  // copy data to host
  cudaMemcpy(py_copy.data(), d_py, py.size() * sizeof(double),
             cudaMemcpyDeviceToHost);
  return py_copy;
}

const std::vector<int> particles_4d_gpu::get_steps() const {
  std::vector<int> steps_copy(steps.size());
  // copy data to host
  cudaMemcpy(steps_copy.data(), d_steps, steps.size() * sizeof(int),
             cudaMemcpyDeviceToHost);
  return steps_copy;
}
...
std::vector<double> particles_4d_gpu::get_radius() const {
  ...
  gpu_evaluate_radius<<<_optimal_nblocks(), 512>>>(d_x, d_px, d_y, d_py,
                                                   d_radius, steps.size());
  // copy data to host
  cudaMemcpy(radius_data.data(), d_radius, steps.size() * sizeof(double),
             cudaMemcpyDeviceToHost);
  // free memory
  cudaFree(d_radius);
  return radius_data;
...
std::vector<double> particles_4d_gpu::get_action() const {
  ...
  gpu_evaluate_action<<<_optimal_nblocks(), 512>>>(d_x, d_px, d_y, d_py,
                                                   d_action, steps.size());
  // copy data to host
  cudaMemcpy(action_data.data(), d_action, steps.size() * sizeof(double),
             cudaMemcpyDeviceToHost);
  // free memory
  cudaFree(d_action);
  return action_data;
...
std::vector<double> particles_4d_gpu::get_action_x() const {
  ...
  cudaMalloc(&d_action, steps.size() * sizeof(double));
  // calculate action data
  gpu_evaluate_action_x<<<_optimal_nblocks(), 512>>>(d_x, d_px, d_y, d_py,
                                                     d_action, steps.size());
  // copy data to host
  cudaMemcpy(action_data.data(), d_action, steps.size() * sizeof(double),
             cudaMemcpyDeviceToHost);
...
std::vector<double> particles_4d_gpu::get_action_y() const {
  ...
  cudaMalloc(&d_action, steps.size() * sizeof(double));
  // calculate action data
  gpu_evaluate_action_y<<<_optimal_nblocks(), 512>>>(d_x, d_px, d_y, d_py,
                                                     d_action, steps.size());
  // copy data to host
  cudaMemcpy(action_data.data(), d_action, steps.size() * sizeof(double),
             cudaMemcpyDeviceToHost);
...
std::vector<double> particles_4d_gpu::get_angles_x() const {
  ...
  gpu_evaluate_angle<<<_optimal_nblocks(), 512>>>(d_x, d_px, d_angles,
                                                  steps.size());
  // copy data to host
  std::vector<double> angles(steps.size());
  cudaMemcpy(angles.data(), d_angles, steps.size() * sizeof(double),
             cudaMemcpyDeviceToHost);
  // free memory
  cudaFree(d_angles);
  return angles;
...
std::vector<double> particles_4d_gpu::get_angles_y() const {
  ...
  cudaMalloc(&d_angles, steps.size() * sizeof(double));
  // calculate angles data
  gpu_evaluate_angle<<<_optimal_nblocks(), 512>>>(d_y, d_py, d_angles,
                                                  steps.size());
  // copy data to host
  std::vector<double> angles(steps.size());
  cudaMemcpy(angles.data(), d_angles, steps.size() * sizeof(double),
...
const std::vector<uint8_t> particles_4d_gpu::get_valid() const {
  ...
  // copy data to host
  cudaMemcpy(valid_copy.data(), d_valid, valid.size() * sizeof(uint8_t),
             cudaMemcpyDeviceToHost);
  return valid_copy;
}
...
const std::vector<uint8_t> particles_4d_gpu::get_ghost() const {
  ...
  // copy data to host
  cudaMemcpy(ghost_copy.data(), d_ghost, ghost.size() * sizeof(uint8_t),
             cudaMemcpyDeviceToHost);
  return ghost_copy;
}
...
const std::vector<size_t> particles_4d_gpu::get_idx() const {
  ...
  // copy data to host
  cudaMemcpy(idx_copy.data(), d_idx, idx.size() * sizeof(size_t),
             cudaMemcpyDeviceToHost);
  return idx_copy;
}
...
const std::vector<size_t> particles_4d_gpu::get_idx_base() const {
  ...
  // copy data to host
  cudaMemcpy(idx_base_copy.data(), d_idx_base, idx_base.size() * sizeof(size_t),
             cudaMemcpyDeviceToHost);
  return idx_base_copy;
}

const size_t &particles_4d_gpu::get_n_particles() const { return n_particles; }

const size_t &particles_4d_gpu::get_n_ghosts_per_particle() const {
  return n_ghosts_per_particle;
}

particles_4d_gpu::~particles_4d_gpu() { _general_cudaFree(); }
...
matrix_4d_vector_gpu::matrix_4d_vector_gpu(size_t _N) {
  ...
  cudaMemcpy(d_matrix, matrix_flattened.data(),
             matrix_flattened.size() * sizeof(double), cudaMemcpyHostToDevice);
}

matrix_4d_vector_gpu::~matrix_4d_vector_gpu() { cudaFree(d_matrix); }
...
void matrix_4d_vector_gpu::structured_multiply(
    const henon_tracker_gpu &tracker, const particles_4d_gpu &particles,
    const double &mu) {
  get_matrices_and_multiply<<<n_blocks, 512>>>(
      d_matrix, particles.d_x, particles.d_px, particles.d_y, particles.d_py,
      tracker.d_omega_x_sin, tracker.d_omega_x_cos, tracker.d_omega_y_sin,
      tracker.d_omega_y_cos, mu, particles.global_steps, N);
}

void matrix_4d_vector_gpu::set_with_tracker(const henon_tracker_gpu &tracker,
                                            const particles_4d_gpu &particles,
                                            const double &mu) {
...
std::vector<std::vector<double>> matrix_4d_vector_gpu::get_vector(
    const std::vector<std::vector<double>> &rv) const {
  std::vector<double> matrix_flattened(N * 16, 0.0);

  // copy matrix from GPU to CPU
  cudaMemcpy(matrix_flattened.data(), d_matrix,
             matrix_flattened.size() * sizeof(double), cudaMemcpyDeviceToHost);

  std::vector<std::vector<std::vector<double>>> matrix(
      N, std::vector<std::vector<double>>(4, std::vector<double>(4, 0.0)));
  ...
  std::vector<std::vector<double>> vectors(matrix.size(),
  ...
  for (size_t i = 0; i < n_threads; i++) {
    threads.push_back(std::thread([&, i]() {
      for (size_t j = i; j < matrix.size(); j += n_threads) {
        ...
        vectors[j][0] = matrix[j][0][0] * rv[j][0] +
                        matrix[j][0][1] * rv[j][1] +
                        matrix[j][0][2] * rv[j][2] + matrix[j][0][3] * rv[j][3];
        vectors[j][1] = matrix[j][1][0] * rv[j][0] +
                        matrix[j][1][1] * rv[j][1] +
                        matrix[j][1][2] * rv[j][2] + matrix[j][1][3] * rv[j][3];
        vectors[j][2] = matrix[j][2][0] * rv[j][0] +
                        matrix[j][2][1] * rv[j][1] +
                        matrix[j][2][2] * rv[j][2] + matrix[j][2][3] * rv[j][3];
        vectors[j][3] = matrix[j][3][0] * rv[j][0] +
                        matrix[j][3][1] * rv[j][1] +
                        matrix[j][3][2] * rv[j][2] + matrix[j][3][3] * rv[j][3];
      }
    }));
  }
...
void matrix_4d_vector_gpu::explicit_copy(const matrix_4d_vector_gpu &other) {
  ...
  assert(other.N == N);
  cudaMemcpy(d_matrix, other.d_matrix, N * 16 * sizeof(double),
             cudaMemcpyDeviceToDevice);
}

const std::vector<std::vector<std::vector<double>>>
matrix_4d_vector_gpu::get_matrix() const {
  std::vector<double> matrix_flattened(N * 16, 0.0);
...
vector_4d_gpu::vector_4d_gpu(const std::vector<std::vector<double>> &vectors) {
  ...
  cudaMemcpy(d_vectors, vectors_flattened.data(), N * 4 * sizeof(double),
             cudaMemcpyHostToDevice);
}

vector_4d_gpu::~vector_4d_gpu() { cudaFree(d_vectors); }

void vector_4d_gpu::set_vectors(
...
const std::vector<std::vector<double>> vector_4d_gpu::get_vectors() const {
  ...
  cudaMemcpy(vectors_flattened.data(), d_vectors, N * 4 * sizeof(double),
             cudaMemcpyDeviceToHost);
...
lyapunov_birkhoff_construct::lyapunov_birkhoff_construct(size_t _N,
                                                         size_t _n_weights)
    : N(_N), n_weights(_n_weights), idx(0) {
  n_blocks = (N + 512 - 1) / 512;

  auto birkhoff = birkhoff_weights(n_weights);
  // copy birkhoff to GPU
  cudaMalloc(&d_birkhoff, n_weights * sizeof(double));
  cudaMemcpy(d_birkhoff, birkhoff.data(), n_weights * sizeof(double),
             cudaMemcpyHostToDevice);
...
void lyapunov_birkhoff_construct::add(const vector_4d_gpu &vectors) {
  ...
  kernel_lyap_birk<<<n_blocks, 512>>>(d_vector, d_vector_b, vectors.d_vectors,
                                      d_birkhoff, N, n_weights, idx);
  idx++;
}

std::vector<double> lyapunov_birkhoff_construct::get_weights() const {
  std::vector<double> weights(n_weights, 0.0);
  cudaMemcpy(weights.data(), d_birkhoff, n_weights * sizeof(double),
             cudaMemcpyDeviceToHost);
  return weights;
}
...
std::vector<double> lyapunov_birkhoff_construct::get_values_raw() const {
  ...
  cudaMemcpy(values.data(), d_vector, N * sizeof(double),
             cudaMemcpyDeviceToHost);
  return values;
}
...
std::vector<double> lyapunov_birkhoff_construct::get_values_b() const {
  std::vector<double> values(N, 0.0);
  // copy values from GPU to CPU
  cudaMemcpy(values.data(), d_vector_b, N * sizeof(double),
             cudaMemcpyDeviceToHost);
  return values;
}

lyapunov_birkhoff_construct_multi::lyapunov_birkhoff_construct_multi(
    size_t _N, std::vector<size_t> _n_weights)
    : N(_N), n_weights(_n_weights), idx(0) {
  n_blocks = (N + 512 - 1) / 512;
  ...
  for (size_t i = 0; i < n_weights.size(); i++) {
    ...
    cudaMemcpy(d_birkhoff[i], birkhoff.data(), n_weights[i] * sizeof(double),
               cudaMemcpyHostToDevice);
...
void lyapunov_birkhoff_construct_multi::add(const vector_4d_gpu &vectors) {
  ...
  for (size_t i = 0; i < n_weights.size(); i++) {
    if (idx < n_weights[i]) {
      kernel_lyap_birk<<<n_blocks, 512>>>(d_vector[i], d_vector_b[i],
                                          vectors.d_vectors, d_birkhoff[i], N,
                                          n_weights[i], idx);
    }
  }
  idx++;
...
std::vector<std::vector<double>>
lyapunov_birkhoff_construct_multi::get_weights() const {
  std::vector<std::vector<double>> weights(n_weights.size());
  for (size_t i = 0; i < n_weights.size(); i++) {
    weights[i].resize(n_weights[i], 0.0);
    cudaMemcpy(weights[i].data(), d_birkhoff[i], n_weights[i] * sizeof(double),
               cudaMemcpyDeviceToHost);
  }
  return weights;
}
...
std::vector<std::vector<double>>
lyapunov_birkhoff_construct_multi::get_values_raw() const {
  std::vector<std::vector<double>> values(n_weights.size());
  for (size_t i = 0; i < n_weights.size(); i++) {
    values[i].resize(N, 0.0);
    // copy values from GPU to CPU
    cudaMemcpy(values[i].data(), d_vector[i], N * sizeof(double),
               cudaMemcpyDeviceToHost);
  }
  return values;
}
...
std::vector<std::vector<double>>
lyapunov_birkhoff_construct_multi::get_values_b() const {
  std::vector<std::vector<double>> values(n_weights.size());
  for (size_t i = 0; i < n_weights.size(); i++) {
    values[i].resize(N, 0.0);
    // copy values from GPU to CPU
    cudaMemcpy(values[i].data(), d_vector_b[i], N * sizeof(double),
               cudaMemcpyDeviceToHost);
  }
  return values;
}
...
void megno_construct::add(const matrix_4d_vector_gpu &matrix_a,
                          const matrix_4d_vector_gpu &matrix_b) {
  // add vectors to lyapunov
  kernel_megno<<<n_blocks, 512>>>(d_layer1, d_layer2, d_layer3,
                                  matrix_a.d_matrix, matrix_b.d_matrix, N, idx);
  idx++;
}
...
std::vector<double> megno_construct::get_values() const {
  std::vector<double> values(N, 0.0);
  // copy values from GPU to CPU
  cudaMemcpy(values.data(), d_layer3, N * sizeof(double),
             cudaMemcpyDeviceToHost);
  return values;
}

megno_birkhoff_construct::megno_birkhoff_construct(
    size_t _N, std::vector<size_t> _turn_samples)
    : N(_N) {
  n_blocks = (N + 512 - 1) / 512;
  idx = 1;

  cudaMalloc(&d_turn_samples, _turn_samples.size() * sizeof(size_t));
  cudaMemcpy(d_turn_samples, _turn_samples.data(),
             _turn_samples.size() * sizeof(size_t), cudaMemcpyHostToDevice);
  ...
  for (size_t i = 0; i < n_turn_samples; i++) {
    ...
    cudaMemcpy(d_weights_i, weights.data(), weights.size() * sizeof(double),
               cudaMemcpyHostToDevice);
    cudaMemcpy(d_weights + i, &d_weights_i, sizeof(double *),
               cudaMemcpyHostToDevice);

    double *d_layer2_i;
    cudaMalloc(&d_layer2_i, N * sizeof(double));
    cudaMemset(d_layer2_i, 0.0, N * sizeof(double));
    cudaMemcpy(d_layer2 + i, &d_layer2_i, sizeof(double *),
               cudaMemcpyHostToDevice);
  }
...
megno_birkhoff_construct::~megno_birkhoff_construct() {
  ...
  for (size_t i = 0; i < n_turn_samples; i++) {
    double *d_weights_i;
    cudaMemcpy(&d_weights_i, d_weights + i, sizeof(double *),
               cudaMemcpyDeviceToHost);
    cudaFree(d_weights_i);

    double *d_layer2_i;
    cudaMemcpy(&d_layer2_i, d_layer2 + i, sizeof(double *),
               cudaMemcpyDeviceToHost);
    cudaFree(d_layer2_i);
  }
...
void megno_birkhoff_construct::add(const matrix_4d_vector_gpu &matrix_a,
                                   const matrix_4d_vector_gpu &matrix_b) {
  // add vectors to lyapunov
  kernel_megno_birkhoff<<<n_blocks, 512>>>(
      d_layer1, d_layer2, d_weights, d_turn_samples, matrix_a.d_matrix,
      matrix_b.d_matrix, N, idx, n_turn_samples);
  idx++;
...
std::vector<std::vector<double>> megno_birkhoff_construct::get_values() const {
  ...
  for (size_t i = 0; i < n_turn_samples; i++) {
    ...
    cudaMemcpy(&d_layer2_i, d_layer2 + i, sizeof(double *),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(values[i].data(), d_layer2_i, N * sizeof(double),
               cudaMemcpyDeviceToHost);
  }
  return values;
}
...
tune_birkhoff_construct::tune_birkhoff_construct(
  ...
  for (size_t i = 0; i < n_turn_samples; i++) {
    ...
    cudaMemcpy(d_tune1_x + i, &d_tune1_x_i, sizeof(double *),
               cudaMemcpyHostToDevice);

    double *d_tune2_x_i;
    cudaMalloc(&d_tune2_x_i, N * sizeof(double));
    cudaMemcpy(d_tune2_x + i, &d_tune2_x_i, sizeof(double *),
               cudaMemcpyHostToDevice);

    double *d_tune1_y_i;
    cudaMalloc(&d_tune1_y_i, N * sizeof(double));
    cudaMemcpy(d_tune1_y + i, &d_tune1_y_i, sizeof(double *),
               cudaMemcpyHostToDevice);

    double *d_tune2_y_i;
    cudaMalloc(&d_tune2_y_i, N * sizeof(double));
    cudaMemcpy(d_tune2_y + i, &d_tune2_y_i, sizeof(double *),
               cudaMemcpyHostToDevice);
  }
...
tune_birkhoff_construct::~tune_birkhoff_construct() {
  ...
  for (size_t i = 0; i < n_turn_samples; i++) {
    ...
    double *d_tune1_x_i;
    cudaMemcpy(&d_tune1_x_i, d_tune1_x + i, sizeof(double *),
               cudaMemcpyDeviceToHost);
    cudaFree(d_tune1_x_i);

    double *d_tune2_x_i;
    cudaMemcpy(&d_tune2_x_i, d_tune2_x + i, sizeof(double *),
               cudaMemcpyDeviceToHost);
    cudaFree(d_tune2_x_i);

    double *d_tune1_y_i;
    cudaMemcpy(&d_tune1_y_i, d_tune1_y + i, sizeof(double *),
               cudaMemcpyDeviceToHost);
    cudaFree(d_tune1_y_i);

    double *d_tune2_y_i;
    cudaMemcpy(&d_tune2_y_i, d_tune2_y + i, sizeof(double *),
               cudaMemcpyDeviceToHost);
...
void tune_birkhoff_construct::first_add(const particles_4d_gpu &particles) {
  kernel_tune_birkhoff_init<<<n_blocks, 512>>>(particles.d_x, particles.d_px,
                                               particles.d_y, particles.d_py,
                                               store_x, store_y, N);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
    std::cerr << "CUDA error: " << cudaGetErrorString(err) << std::endl;
...
void tune_birkhoff_construct::add(const particles_4d_gpu &particles) {
  kernel_tune_birkhoff<<<n_blocks, 512>>>(
      particles.d_x, particles.d_px, particles.d_y, particles.d_py, store_x,
      store_y, d_tune1_x, d_tune2_x, d_tune1_y, d_tune2_y, d_weights,
      d_turn_samples, N, idx, n_turn_samples);
  idx++;
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
    std::cerr << "CUDA error: " << cudaGetErrorString(err) << std::endl;
...
std::vector<std::vector<double>> tune_birkhoff_construct::get_tune1_x() const {
  ...
  for (size_t i = 0; i < n_turn_samples; i++) {
    out_vec[i].resize(N);
    double *d_tune_i;
    cudaMemcpy(&d_tune_i, d_tune1_x + i, sizeof(double *),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(out_vec[i].data(), d_tune_i, N * sizeof(double),
               cudaMemcpyDeviceToHost);
  }
  return out_vec;
}
...
std::vector<std::vector<double>> tune_birkhoff_construct::get_tune2_x() const {
  ...
  for (size_t i = 0; i < n_turn_samples; i++) {
    out_vec[i].resize(N);
    double *d_tune_i;
    cudaMemcpy(&d_tune_i, d_tune2_x + i, sizeof(double *),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(out_vec[i].data(), d_tune_i, N * sizeof(double),
               cudaMemcpyDeviceToHost);
  }
...
std::vector<std::vector<double>> tune_birkhoff_construct::get_tune1_y() const {
  ...
  for (size_t i = 0; i < n_turn_samples; i++) {
    out_vec[i].resize(N);
    double *d_tune_i;
    cudaMemcpy(&d_tune_i, d_tune1_y + i, sizeof(double *),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(out_vec[i].data(), d_tune_i, N * sizeof(double),
               cudaMemcpyDeviceToHost);
  }
...
std::vector<std::vector<double>> tune_birkhoff_construct::get_tune2_y() const {
  ...
  for (size_t i = 0; i < n_turn_samples; i++) {
    out_vec[i].resize(N);
    double *d_tune_i;
    cudaMemcpy(&d_tune_i, d_tune2_y + i, sizeof(double *),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(out_vec[i].data(), d_tune_i, N * sizeof(double),
               cudaMemcpyDeviceToHost);
  }
...
henon_tracker_gpu::henon_tracker_gpu(unsigned int n_turns, double omega_x,
                                     double omega_y,
                                     std::string modulation_kind,
                                     double omega_0, double epsilon, int offset)
    : henon_tracker(n_turns, omega_x, omega_y, modulation_kind, omega_0,
                    epsilon, offset) {
  ...
  cudaMemcpy(d_omega_x_sin, omega_x_sin.data(),
             omega_x_sin.size() * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(d_omega_x_cos, omega_x_cos.data(),
             omega_x_cos.size() * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(d_omega_y_sin, omega_y_sin.data(),
             omega_y_sin.size() * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(d_omega_y_cos, omega_y_cos.data(),
             omega_y_cos.size() * sizeof(double), cudaMemcpyHostToDevice);
}
...
void henon_tracker_gpu::compute_a_modulation(unsigned int n_turns,
                                             double omega_x, double omega_y,
                                             std::string modulation_kind,
                                             double omega_0, double epsilon,
                                             int offset) {
  henon_tracker::compute_a_modulation(
...
void henon_tracker_gpu::track(particles_4d_gpu &particles, unsigned int n_turns,
                              double mu, double barrier, double kick_module,
                              bool inverse) {
  ...
  for (unsigned int j = 0; j < n_turns; j++) {
    gpu_henon_track<<<particles._optimal_nblocks(), 512>>>(
        ...
        particles.d_steps, particles.x.size(), 1, barrier * barrier, mu,
        d_omega_x_sin, d_omega_x_cos, d_omega_y_sin, d_omega_y_cos, kick_module,
        particles.d_rng_state, inverse, allowed_steps);
...
henon_tracker_gpu::get_tangent_matrix(const particles_4d_gpu &particles,
                                      const double &mu,
  ...
  for (unsigned int i = 0; i < x.size(); i++) {
    if (valid[i]) {
      if (!reverse) {
        result[i] = tangent_matrix(x[i], px[i], y[i], py[i], omega_x_sin[steps],
                                   omega_x_cos[steps], omega_y_sin[steps],
                                   omega_y_cos[steps], mu);
      } else {
        result[i] = inverse_tangent_matrix(
            x[i], px[i], y[i], py[i], omega_x_sin[steps], omega_x_cos[steps],
            omega_y_sin[steps], omega_y_cos[steps], mu);
      }
    } else {
      result[i] = std::vector<std::vector<double>>(
          4, std::vector<double>(4, std::numeric_limits<double>::quiet_NaN()));
    }
  }
...
void storage_4d_gpu::store(const particles_4d_gpu &particles) {
  cudaMemcpy(d_x + idx * N, particles.d_x, N * sizeof(double),
             cudaMemcpyDeviceToDevice);
  cudaMemcpy(d_px + idx * N, particles.d_px, N * sizeof(double),
             cudaMemcpyDeviceToDevice);
  cudaMemcpy(d_y + idx * N, particles.d_y, N * sizeof(double),
             cudaMemcpyDeviceToDevice);
  cudaMemcpy(d_py + idx * N, particles.d_py, N * sizeof(double),
             cudaMemcpyDeviceToDevice);
...
const std::vector<std::vector<double>> storage_4d_gpu::get_x() const {
  ...
  cudaMemcpy(temp.data(), d_x, N * batch_size * sizeof(double),
             cudaMemcpyDeviceToHost);
...
const std::vector<std::vector<double>> storage_4d_gpu::get_px() const {
  std::vector<std::vector<double>> result(batch_size, std::vector<double>(N));
  std::vector<double> temp(N * batch_size, 0);
  cudaMemcpy(temp.data(), d_px, N * batch_size * sizeof(double),
             cudaMemcpyDeviceToHost);
...
const std::vector<std::vector<double>> storage_4d_gpu::get_y() const {
  std::vector<std::vector<double>> result(batch_size, std::vector<double>(N));
  std::vector<double> temp(N * batch_size, 0);
  cudaMemcpy(temp.data(), d_y, N * batch_size * sizeof(double),
             cudaMemcpyDeviceToHost);
...
const std::vector<std::vector<double>> storage_4d_gpu::get_py() const {
  std::vector<std::vector<double>> result(batch_size, std::vector<double>(N));
  std::vector<double> temp(N * batch_size, 0);
  cudaMemcpy(temp.data(), d_py, N * batch_size * sizeof(double),
             cudaMemcpyDeviceToHost);
